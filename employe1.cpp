...
bool employe1::ajouter()
{
   ...
   QSqlQueryModel * employe1::afficher()
 {
    ...
    bool employe1 :: modifier (employe1 b)
    {
        ...
        query.prepare("UPDATE EMPLOYE SET NOM=:NOM,SALAIRE=:SALAIRE,PRENOM=:PRENOM,TELEPHONE=:TELEPHONE,ID=:ID,FORMATION=:FORMATION,AGE=:AGE WHERE ID=:ID");
 ...
 /*    bool archiver(employe1 e, QString NOM) {
         ...
         query.prepare("delete from Opinion where CIN=?");
         query.addBindValue(NOM);



         bool res1=query.exec() ;
         qDebug()<<" "<<res1 ;
         return res1 ;
